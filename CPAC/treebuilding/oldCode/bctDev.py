...
def get_ordered_property(p, corecat, sorted_indices_this, row, current_snap,
                         first_siblings, next_siblings, parent_tags, particle_mass=particle_masses[sim]):
...
def add_properties_to_tree(core_tag, location, coretree, corecat,
                           ...
                           current_snap, parent_fof_tag, particle_mass=particle_masses[sim]):
...
def main(argv):
    ...
    ncore_max = int(argv.get(5, 6776000)) #for serial code (number of cores)/100
...
def drawforest(trees, treenum, filetype='.png', particle_mass=particle_masses[sim], cmap='Purples',
    ...
    treegraph = pydot.Dot(graph_type="digraph", compound='true', rankdir="LR",
                          labelloc="c", labeljust="c", style="filled") #draws top to bottom (rankdir=BT/TB draw LR)
    clusters = []
    nodes = []
    node_names= []
    ...
    for sidx, snap in enumerate(snaps):
        ...
        if not MTtrees:  #add infall halo mass of fof group to label
            ...
            #colors_rgb =[tuple(list(c[:3]) + [alpha]) for c in colors_rgb] #no change
            colors_cl = [colors.to_hex(c) for c in colors_rgb]
        else:
            colors_cl = [color_cl for f in fof_groups]
        cl_this = [pydot.Cluster('FoF'+str(fof_groups[f]), label=labels_cl[f],
                                 color=colors_cl[f], fontcolor=clfont) for f in range(len(fof_groups))] 
#                                 label='FoF #{}; Coretag = {}'.format(str(f), tree[MostBoundID_Coretag][f]),
#                                 color=color_cl, fontcolor=clfont) for f in fof_groups]
#                                 rankdir='LR', rank='same', color=color_cl) for f in fof_groups]
            ...
            for nidx, (node, next_node_loc, next_prog_loc) in enumerate(zip(nodes_fof, next_halos[mask], next_progs)):
                ...
                if next_prog_loc != -1:
                    if next_prog_loc in locs_in_fof:   #is next progenitor in current snap in this fof_group?
                        ...
                        print('Warning: next progenitor {} for node {} not found in current fof group in snap {}'.format(next_prog_loc,
                                                                                                                         node.get_name(), snap))
                    else: # current next progenitor points to node in some previous snap
                        name = str(next_prog_loc)
                        node_loc = [(n, node_names[n].index(name)) for n in range(len(node_names)) if name in node_names[n]]
                        if node_loc:
                            #add edge from current node to next progenitor
                            treegraph.add_edge(pydot.Edge(node, nodes[node_loc[0][0]][node_loc[0][1]], color=next_prog_arrow))
                            #replace this next progenitor with -1 for consistency check at end of code
                            next_progenitors_this[next_progenitors_this.index(int(next_prog_loc))] = -1                        
                            
            #add edges for descendents progenitors and next progenitors (connect with previous snapshots)
            if sidx > 0:
        ...
        #final case for next progenitors: progenitors in same snapshot but in different fof group
