...
def clean(corecat, sorted_coretags):
    ...
    ''' 
    for k in list(corecat.keys()):
        if k=='infall_fof_halo_mass' or k=='infall_tree_node_mass':
            print("Len before:",len(corecat[k]))
            delIndices = np.where(corecat[k]<100)[0]
            temp = np.delete(corecat[k].copy(),delIndices)
            corecat[k]=temp
            print("Len after:",len(corecat[k]))
    for k in list(corecat.keys()):       
        if k=='infall_fof_halo_mass' or k=='infall_tree_node_mass':
            continue
        else:
...
def add_properties_to_tree(core_tag, location, coretree, corecat,
    ...
    print("\n\n\nCIRETREE",coretree[core_tag],coretree[Descendent])
...
def validate_trees(counts, coretrees, start, end):
    ...
    for count, col in zip(counts, np.arange(start, end)):
        print(coretrees[MostBoundID_Coretag].T[col][0:count])
...
def main(argv):
    ...
    for n, s in enumerate(snapshots): #process in descending order
        ...
        print("LEN:",len(snapshots))
        stime = time() 
        delIndices, corecat = get_core_snapshot(coredir, int(s))
        delIndicesArr.append(delIndices)
        if corecat:
            ...
            if n == 0:
                ...
                if vector: # initialize dict of matrices
                    ...
                    ctime = time()
                    for i,p in enumerate(float_properties):
                        print("Float:",i,"/",len(float_properties)-1)
                        coretrees[p] = np.array([no_float]*Ncores*len(snapshots)).reshape(len(snapshots), Ncores)
                    for i,p in enumerate(integer_properties):
                        print("Int:",i,"/",len(integer_properties)-1)
                    ...
                    print(coretrees['MostBoundID->Coretag'])
                else:
