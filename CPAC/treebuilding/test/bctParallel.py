...
def get_core_snapshot(coredir, snapshot, template=cc_template):
    ...
    if os.path.exists(fn):
        ...
        for k in keys + [mkey]:
            if k=='infall_fof_halo_mass' or k=='infall_tree_node_mass':
                data[k] = coredata[k][()]/particle_mass
            else:
                data[k] = coredata[k][()]
    else:
...
def orderedIntArrays(integer_property):
    print('Running int property:',integer_property)
    
    coretrees={}
    coretrees[integer_property] = np.array([-999.0]*6996900*101).reshape(101, 6996900)

    n_bytes = 2**31
    max_bytes = 2**31 - 1
    data = bytearray(n_bytes)

    bytes_out = pickle.dumps(coretrees,protocol=4)
    with open('{}.txt'.format(integer_property), 'wb') as f_out:
        for idx in range(0, len(bytes_out), max_bytes):
            f_out.write(bytes_out[idx:idx+max_bytes])
    f_out.close()
    print('Done writing',integer_property)

def orderedFloatArrays(float_property):
    print('Running float property:',float_property)
    
    coretrees={}
    coretrees[float_property] = np.array([-999.0]*6996900*101).reshape(101, 6996900)
    ...
    bytes_out = pickle.dumps(coretrees,protocol=4)
    with open('{}.txt'.format(float_property), 'wb') as f_out:
        for idx in range(0, len(bytes_out), max_bytes):
            f_out.write(bytes_out[idx:idx+max_bytes])
    print('Done writing',float_property)
...
def main(argv):
    ...
    for n, s in enumerate(snapshots): #process in descending order
        ...
        if corecat:
            ...
            #corecat[coremass] /= particle_mass
            #print(corecat[coremass])
            if n == 0:
                ...
                if vector: # initialize dict of matrices
                    ...
                    pool.starmap(orderedIntArrays,zip(integer_properties))
                    pool.starmap(orderedFloatArrays,zip(float_properties))
                    ...
                    for prop in integer_properties:
                      file_path='{}.txt'.format(prop)
                      input_size = os.path.getsize(file_path)
                      with open(file_path, 'rb') as f_in:
                          for _ in range(0, input_size, max_bytes):
                              bytes_in += f_in.read(max_bytes)
                      coretrees[prop] = pickle.loads(bytes_in).pop('Descendent')
                    ...
                    for prop in float_properties:
                        ...
                        with open(file_path, 'rb') as f_in:
                            for _ in range(0, input_size, max_bytes):
                                bytes_in += f_in.read(max_bytes)
                        coretrees[prop] = pickle.loads(bytes_in).pop('Pos_x')
...
def drawforest(trees, treenum, filetype='.png', particle_mass=particle_mass_AQ, cmap='YlOrRd',
    ...
    treegraph = pydot.Dot(graph_type="digraph", compound='true', rankdir="BT", labelloc="c", labeljust="c", ranksep=0.2, style="filled")
    clusters = []
    nodes = []
    ...
    for sidx, snap in enumerate(np.arange(np.min(tree[SnapNum]), np.max(tree[SnapNum]+1))[::-1]):
        ...
        for cl, fof_count in zip(cl_this, fof_counts):
            ...
            for nidx in np.arange(fof_count):
                ...
                node = pydot.Node(node_name, style="filled", label=node_label,color=outlinecolor, fontcolor=fontcolor)
                cl.add_node(node)
