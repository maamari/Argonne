...
def main(argv):
    ...
    for n, s in enumerate(tqdm(snapshots)): #process in descending order
...
def getnodecolor(mass, logm_min, logm_max, n_min=20, particle_mass=particle_masses[sim]):
...
def drawforest(trees, treenum, filetype='.png', particle_mass=particle_masses[sim], cmap='YlOrRd',
    ...
    for sidx, snap in enumerate(np.arange(np.min(tree[SnapNum]), np.max(tree[SnapNum]+1))[::-1]):
        ...
        for cl, fof_count in zip(cl_this, fof_counts):
            ...
            for nidx in np.arange(fof_count):
                ...
                node_label = "Node {}, Snap {}\n id={}\n M={}".format(locs[lidx], snap, coretags[lidx], masses[lidx]/2.7e9)
