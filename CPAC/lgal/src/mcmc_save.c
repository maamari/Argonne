  ...
  for(snap=0;snap<NOUT;snap++)
    {
	 ...
	 //printf("%lld==%lld, snap=%d,%d\n", HaloIDs[HaloGal[gal_index].HaloNr].FirstHaloInFOFgroup, MCMC_FOF[fof].FoFID[snap],snap,fof);
	 if(StellarMass > low_mass_limit &&
	   StellarMass < high_mass_limit &&
	   fof==0)
	   //HaloIDs[HaloGal[gal_index].HaloNr].FirstHaloInFOFgroup == MCMC_FOF[fof].FoFID[snap])
	  {
	    ...
	    //printf("\nSnapNum:%d\n",snap);
	    //for(int i=0; i<7; i++)
		//    printf("%f\n",lum_to_mag(HaloGal[gal_index].LumDust[i][snap]));
